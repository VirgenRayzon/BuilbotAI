...
import { Cpu, X as CloseIcon, Plus } from "lucide-react";
...
export function BuildItemStandalone({
  name,
  label,
  component,
  icon: Icon,
  ...
  if (components.length === 0) {
    return (
      <div 
        className={cn(
          "flex items-center justify-between py-2 rounded-xl px-3 -mx-1 transition-all duration-300 border border-dashed",
          onSelect ? "cursor-pointer" : "cursor-default",
          isActiveFilter 
            ? "opacity-100 bg-primary/5 border-primary/30 shadow-[0_0_15px_rgba(34,211,238,0.1)]"
            : "opacity-60 border-border/30 hover:opacity-100 hover:border-primary/30 hover:bg-primary/[0.03]"
        )}
      ...
      >
        ...
        <Button 
          size="sm" 
          variant="ghost" 
          className="h-7 px-3 text-[9px] uppercase font-bold text-primary border border-primary/20 hover:bg-primary/10 transition-all rounded-lg flex items-center gap-1"
        >
